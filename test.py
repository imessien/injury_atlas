...
class ModelEnsemble:
    ...
    def prepare_training_data(
        adata: sc.AnnData, target_cell_type: str, perturbation_type: str = "knockout"
        ...
        if perturbation_type not in all_models.CONFIG["perturbation_types"]:
            raise ValueError(
                f"Perturbation type must be one of {all_models.CONFIG['perturbation_types']}"
            )

        filtered_data = adata[
            ~(
                (adata.obs[all_models.CONFIG["cell_type_key"]] == target_cell_type)
                & (
                    adata.obs[all_models.CONFIG["condition_key"]]
                    == all_models.CONFIG["stim_key"]
                )
            )
        ]
        ...
        return (
            filtered_data[train_idx],
            filtered_data[val_idx],
            filtered_data[holdout_idx],
        )
    ...
    def train_and_evaluate(
        ...
        try:
            if model_name == "Geneformer":
                ...
                model.train(
                    max_epochs=epochs, batch_size=32, early_stopping=True, patience=10
                )
                pred = model.predict(perturbation_type=perturbation_type)

            elif model_name == "scLAMBDA":
                model.train(
                    epochs=epochs, batch_size=32, early_stopping=True, patience=10
                )
                pred = model.predict(perturbation_type=perturbation_type)
            ...
            val_ground_truth = val_data[
                val_data.obs[TrainableModels.CONFIG["cell_type_key"]]
                == target_cell_type
            ]
            ...
            logging.error(
                f"Error during {model_name} training and evaluation: {str(e)}"
            )
            raise
    ...
    for model_name in TrainableModels.MODELS.keys():
        for perturbation_type in TrainableModels.CONFIG["perturbation_types"]:
            ...
            train_data, val_data, holdout_data = TrainableModels.prepare_training_data(
                train_new, perturbation_type=perturbation_type
            )
            ...
            logging.info(
                f"{model_name} {perturbation_type} validation score: {val_score}"
            )
            logging.info(
                f"{model_name} {perturbation_type} holdout score: {holdout_score}"
            )
            ...
            for gene, importance in zip(
                xgb_top_genes, xgb_mean_shap[xgb_top_genes_idx]
            ):
                logging.info(f"{gene}: {importance:.4f}")

            logging.info("\nTop 10 important genes identified by Random Forest:")
            for gene, importance in zip(
                rf_top_genes, rf_feature_importances[rf_top_genes_idx]
            ):
                logging.info(f"{gene}: {importance:.4f}")
